...
\begin{abstract}
This paper introduces a mathematically rigorous and experimentally grounded model of consciousness based on quaternionic Hilbert spaces \cite{tobar2014quaternionic,Moretti2017Quaternionic,Giardino2020Quaternionic}, entropy field dynamics \cite{tononi2016integrated,Jha2025Entropy,Lugten2024Entropy}, and toroidal manifold organization \cite{Hafting2005,Burak2009}. The model constrains the structure, stability, and invariance of an underlying operator that governs conscious state dynamics. Empirical validation using EEG data from spatial navigation and resting state tasks demonstrates stable collapse structure with characteristic spectral gaps and entropy flow patterns. The framework is compatible with multiple physical realizations, including fast microphysical processes hypothesized in the literature \cite{Jepsen2011,Zhang2021,Huang2022}, though no absolute frequency scale is assigned from the operator structure alone. This work seeks to close the explanatory gap \cite{chalmers1995facing} between objective neural processes and subjective experience by establishing operator level causality as the primary explanatory mechanism.
...
The framework proposes that consciousness emerges through operator level dynamics whose physical realization remains an open empirical question. While fast microphysical processes including terahertz range dynamics have been hypothesized in related literature \cite{Jepsen2011,Zhang2021}, this work does not assign an absolute frequency scale to the underlying control process. Instead, the model constrains the geometric and algebraic structure of conscious state evolution, within which multiple physical implementations remain viable. Intersubjective validation is achieved through measurable EEG coherence patterns and entropy gradients that reflect the operator structure, establishing objective correlates of subjective experience without requiring direct frequency measurement.
...
Quantum approaches to consciousness, particularly Orchestrated Objective Reduction \cite{Hameroff1996OrchOR,penrose1989emperor,hameroff2014consciousness}, suggest that consciousness emerges from quantum processes in neural microtubules. These theories posit that non-computable quantum mechanisms are required for consciousness, drawing on Gödel's incompleteness theorems \cite{godel1931formal} and quantum mechanics. However, quantum theories face significant challenges concerning decoherence in warm, noisy biological environments. EntPTC avoids these issues by operating at the classical field level while incorporating quantum inspired mathematical structures through quaternionic dynamics. The model remains agnostic about the absolute physical scale of the generative process, allowing compatibility with multiple microphysical hypotheses without requiring quantum coherence.
...
\]
which lies in the terahertz domain. This is a theoretical prediction, not a measured result from EEG data.

\paragraph{Interpretation.}
This calculation illustrates a theoretical prediction linking operator eigenvalue structure to hypothesized microphysical timescales. The eigenvalue spectrum of the recursive matrices provides dimensionless signatures that could, in principle, constrain future microphysical models. This theoretical bridge requires direct experimental validation at microphysical scales, beyond the scope of current EEG measurements.
...
The Clifford algebra $\mathrm{Cl}(3,0)$ is generated by basis vectors $\{e_1, e_2, e_3\}$ satisfying the relation $e_i e_j + e_j e_i = 2\delta_{ij}$ \cite{Hitzer2013}. The algebra has a graded structure of dimension $2^3 = 8$, consisting of scalars (grade 0), vectors (grade 1), bivectors (grade 2), and a pseudoscalar (grade 3). In the EntPTC model, this algebra serves as the global integration layer where bivectors encode semantic and relational information geometrically \cite{BayroCorreochano2001}. Quaternions, which are isomorphic to the even subalgebra of $\mathrm{Cl}(3,0)$, act as the local stability layer.
...
The EntPTC model generates specific, testable predictions that distinguish it from alternative theories of consciousness \cite{tononi2008consciousness,chalmers1995facing}. The progenitor matrix structure (Figure~\ref{fig:main_progenitor_matrix}) yields eigenvalue spectra providing dimensionless structural invariants for experimental validation. QEEG eigenvalue decomposition offers a method for characterizing operator level dynamics and entropy flow patterns \cite{Huang2022,Sakkalis2011}.
...
\textbf{Against IIT:} EntPTC predicts that consciousness correlates with entropy gradients rather than with integrated information $\Phi$. If high $\Phi$ systems such as photodiodes show no operator level coherence while low $\Phi$ systems with strong entropy gradients do, IIT is falsified in favor of EntPTC.
...
\textbf{Direct Falsification Tests:} (1) If operator level predictions (eigenvalue structure, spectral gaps, entropy flow patterns) fail to correspond to observed EEG coherence dynamics, EntPTC is falsified. (2) If quaternionic non commutativity is absent in perceptual tasks, the mathematical framework is invalidated. (3) If toroidal grid cell disruption has no impact on conscious spatial representation, the topological foundation fails.

These experiments serve as indirect measurements of the Phi linked field dynamics predicted by the model. Clinical protocols involving operant conditioning with QEEG monitoring have been used as indirect readouts of field coherence, matching the write and read logic described by the EntPTC framework. These real world clinical applications provide early validation of the model's central mechanisms.
...
The model posits a hierarchical control structure underlying consciousness, defined by the operator eigenspectrum and collapse dynamics. No absolute frequency scale is assigned; the control process is characterized by dimensionless structural invariants including spectral gaps, decay rates, and entropy flow patterns. Observable EEG dynamics reflect projections of this underlying structure into the measurement domain, shaped by the biophysical medium and recording apparatus.

The operator level control provides a potential physical basis for degrees of free will. When the collapse structure achieves stable coherence, it enables the conscious field component to exert control over biologically driven processes. This is not simple behavioral training but rather the establishment of a coherent channel through which consciousness can modulate biological impulses. The model does not assume consciousness to be a single, monolithic entity. Rather, consciousness is understood as a complex field composed of multiple interacting components, where the inner true self represents the recursive, self aware aspect that gains causal efficacy through operator level coherence. This framework suggests that free will emerges not from randomness or deterministic computation alone, but from the coherent coupling between the conscious field and the biological substrate, mediated by the operator collapse structure.
...
EntPTC makes specific testable predictions at the operator level: (1) stable eigenvalue spectrum with characteristic spectral gaps and decay patterns; (2) entropy flow gradients with specific directional structure on the toroidal manifold; (3) phase coherence patterns in EEG that reflect the underlying collapse structure. These predictions can be tested using EEG, entropy analysis, and topological data analysis methods. The model is compatible with multiple physical realizations at different scales, including hypothesized fast microphysical processes \cite{Jepsen2011,Zhang2021}, though no absolute frequency assignment is made from the operator structure alone.
...
This relation provides a theoretical bridge between the recursive entropy collapse model and hypothesized microphysical resonance frequencies. To illustrate the theoretical prediction, consider a representative eigenvalue with imaginary component
\[
\text{Im}(\lambda) = 8.8 \times 10^{12}\ \text{rad/s}.
\]
...
Future iterations should incorporate stochastic toroidal manifolds with adaptive curvature to model observed entorhinal distortions, extend the entropy operator to handle non differentiable transitions as in Northoff's temporo spatial discontinuity model [150], and generalize the quaternionic algebra toward octonionic or Clifford domains [33, 239]. Experimental work should prioritize QEEG eigenvalue decomposition [181], entropy gradient mapping, and topological data analysis to test the model's operator level predictions. Within these constraints, EntPTC offers a unified, falsifiable description of consciousness as an entropic toroidal field filtered through quaternionic rotation.

\section{Conclusion and Future Directions}

This paper presents the mathematical core of the Entropic Toroidal Consciousness framework, Solution Set A, defining consciousness as the recursive transformation of information across entropy gradients within a quaternionic Hilbert space. By uniting toroidal topology, entropic filtering, and quaternionic rotation, the model provides an analytic bridge between operator level dynamics and phenomenological experience.

The progenitor matrix formalism defines stable collapse modes through dimensionless eigenvalue structure, spectral gaps, and entropy flow patterns. These operator level invariants constrain the dynamics of conscious state evolution and provide testable predictions through EEG coherence, entropy gradients, and topological structure. This work reframes the study of consciousness as a testable, quantitative domain grounded in physics, mathematics, and neurobiology.
...
These structural invariants provide dimensionless signatures of operator collapse dynamics. The theory predicts that if microphysical THz resonance mechanisms exist (as hypothesized in microtubular models), they would manifest as specific eigenvalue patterns. This is a theoretical prediction requiring direct microphysical validation, not a measured result from EEG data.
...
\subsection{D.1 Theoretical Microphysical Timescale Prediction}
The theory predicts characteristic timescales from the toroidal manifold geometry. If microphysical wave dynamics exist on a 3-torus, the wave equation would be
\begin{equation}
\nabla^2 \Psi = \frac{1}{c^2} \frac{\partial^2 \Psi}{\partial t^2}.
\end{equation}
Solutions would have the form $\Psi(x,t) = \Psi_0 e^{i(k \cdot x - \omega t)}$, where the wave vector $k$ is quantized by periodic boundary conditions. Hypothetical resonant frequencies would be
\begin{equation}
\omega_{n,m,l} = \frac{c}{R}\sqrt{n^2 + \left(\frac{mR}{r}\right)^2 + l^2},
\end{equation}
where $R$ and $r$ are the major and minor radii of the torus, and $n,m,l$ are integer indices. For neural tissue parameters ($R \approx 10$ mm, $r \approx 2$ mm), this yields theoretical predictions at 1.00, 1.40, 2.20, and 2.40 THz. This is a theoretical hypothesis requiring microphysical validation, not a measured result.

\subsection{D.2 Structural Invariants and Theoretical Correspondence}
The operator collapse dynamics are characterized through dimensionless structural invariants. The eigenvalue spectrum from the Progenitor Matrix collapse yields scale-invariant ratios and gaps. The key structural invariants include:
\begin{align}
...
\end{align}
The theory predicts that if microphysical THz resonance mechanisms exist (as hypothesized in microtubular models), they would manifest as specific patterns in these dimensionless invariants. This is a theoretical structural correspondence requiring direct microphysical validation, not a measured result from EEG data.
...
\subsection{D.4 Theoretical Timescale Prediction from Operator Eigenvalues}
Equation~\eqref{eq:collapse_frequency} provides a theoretical bridge between the recursive entropy collapse model and hypothesized microphysical timescales. For a representative eigenvalue with imaginary component
\[
...
\]
which lies in the terahertz domain.
...
\]
These dimensionless ratios and decay exponents characterize the operator collapse dynamics. The decay pattern reflects coherence loss under recursive entropy filtering, providing testable signatures through EEG eigenvalue analysis.
...
A theoretical prediction linking operator eigenvalue structure to hypothesized microphysical timescales,
$f = |\mathrm{Im}(\lambda)|/(2\pi)$, where $\mathrm{Im}(\lambda)$ characterizes the operator timescale. This yields theoretical predictions in the terahertz domain, requiring microphysical validation.

\medskip
\textbf{Eigenvalue Imaginary Part.}
For $\lambda = a + ib$, the quantity $b$ characterizes the operator timescale. In EntPTC it provides a theoretical link to hypothesized microphysical dynamics through the collapse frequency condition.

\medskip
\textbf{Entropy Collapse.}
A transition from high dimensional recursive states toward lower dimensional attractors under threshold conditions on system entropy. This process is characterized by operator eigenvalue structure and entropy flow patterns.

\medskip
\textbf{Recursive Attractor Network.}
A system of coupled states organized in toroidal or quaternionic topology, capable of filtering information across entropy gradients through recursive operator dynamics. Recursive attractors form the mathematical substrate of subconscious dynamics in the EntPTC model.
...
The model makes theoretical predictions about hypothesized microphysical timescales near 1.0, 1.4, 2.2, and 2.4 THz. These are not measured results from EEG data but theoretical predictions requiring microphysical validation. Current EEG measurements provide operator level signatures through eigenvalue structure, spectral gaps, and entropy flow patterns. Direct microphysical validation would require emerging methods such as graphene based THz probes and ultrafast spectroscopy, which face sensitivity and SNR limits \cite{Jepsen2011,Huang2022,Krausz2009}. Current validation relies on operator level coherence signatures such as QEEG eigenvalue structure \cite{Sakkalis2011}.
...
\hline
THz theoretical prediction & Jepsen2011, Zhang2021 & Yes & Spectroscopy context for hypothesis \\
\hline
