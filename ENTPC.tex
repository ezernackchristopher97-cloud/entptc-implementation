...
\end{align}
\subsection{Eigenvalue to THz Frequency Mapping}

The transition from eigenvalues to terahertz frequencies follows from the field oscillation equation. The consciousness field $\Phi(\mathbf{r},t)$ satisfies the wave equation:
\begin{align}
\frac{\partial^2 \Phi}{\partial t^2} &= c_\Phi^2 \nabla^2 \Phi - \gamma \frac{\partial \Phi}{\partial t}, \label{eq:31}\\
\text{where } c_\Phi &= \frac{c}{\sqrt{\epsilon_r}} \text{ is the field propagation velocity.} \label{eq:field_velocity}
\end{align}

By substituting representative parameters into Eq.~\eqref{eq:31}, the entropy temporal coupling yields specific values for stability across neural scales \cite{Zhang2021,Huang2022}.

\paragraph{Numeric Examples.}
With representative parameters substituted into Eq.~\eqref{eq:31}:
Example 1: $\epsilon_r = 80$, $\lambda_{\max}(\mathcal{M}) = 0.95$ gives a predicted envelope near $1.4$ THz.  
Example 2: $\epsilon_r = 60$, $\lambda_{\max}(\mathcal{M}) = 0.74$ gives a predicted envelope near $1.1$ THz.  
These examples are illustrative; precise calibration depends on the experimental $\alpha$ scaling inferred from material parameters \cite{Zhang2021,Huang2022}.

For harmonic solutions $\Phi(\mathbf{r},t) = \Phi_0 e^{i(\mathbf{k}\cdot\mathbf{r} - \omega t)}$, the dispersion relation becomes:
\begin{align}
\omega^2 &= c_\Phi^2 k^2 - i\gamma\omega, \label{eq:dispersion}\\
\text{for } \gamma \ll \omega: \quad \omega &\approx c_\Phi |\mathbf{k}|. \label{eq:frequency}
\end{align}

The eigenvalues $\lambda_i$ of $\mathcal{M}_{\Phi\Psi}$ determine structural invariants (ratios, gaps, decay exponents) that are compared to predictions from microtubular THz resonance physics. When the dimensionless eigenvalue structure matches the predicted patterns, a THz control layer operating at characteristic frequencies (1.0, 1.4, 2.2, 2.4 THz) is inferred. This inference is based on structural correspondence, not frequency-domain conversion. The scale separation between EEG dynamics (Hz range) and the inferred THz control layer (THz range) is preserved; no direct mapping between these regimes is performed.
...
\subsection{Eigenvalue Analysis and THz Predictions}

The eigenvalue problem $\mathcal{M}_{\Phi\Psi}\mathbf{v} = \lambda \mathbf{v}$ yields the characteristic frequencies of consciousness field oscillations:
\begin{align}
\lambda_1   &= -0.0500 \pm 0.0200i \, \text{s}^{-1} \quad \text{(Fundamental consciousness mode)}, \\
\lambda_2   &= -0.0750 \pm 0.0150i \, \text{s}^{-1} \quad \text{(Toroidal coupling mode)}, \\
\lambda_3   &= -0.1200 \pm 0.0300i \, \text{s}^{-1} \quad \text{(Quaternionic binding mode)}, \\
&\vdots \\
\lambda_{16} &= -2.4000 \pm 0.1000i \, \text{s}^{-1} \quad \text{(High frequency qualia mode).}
\end{align}

The THz control layer is inferred through structural invariant matching, not through direct frequency conversion. The eigenvalue ratios and spectral gaps from the Progenitor Matrix collapse are compared to dimensionless structural patterns predicted by microtubule THz resonance physics. When these scale-invariant structures align, the model infers the presence of a THz control layer operating at characteristic frequencies of 1.0, 1.4, 2.2, and 2.4 THz, consistent with THz spectroscopy studies of microtubular structures \cite{Zhang2021,Huang2022,Jepsen2011}. This inference does not involve converting EEG frequencies to THz; rather, it identifies structural signatures in the eigenspectrum that correspond to the predicted control layer architecture.
...
\begin{figure}[htbp]
    ...
    \caption{Eigenvalue spectrum decay of the progenitor matrix showing characteristic frequencies mapping to terahertz signatures. The decay pattern follows the recursive entropy filtering mechanism described in Section~\ref{sec:matrix_analysis}.}
    \label{fig:eigenvalue_spectrum}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/thz_frequency_mapping_final.png}
    \caption{Inferred terahertz control layer signatures identified through structural invariant matching. The dimensionless eigenvalue ratios and spectral gaps correspond to predicted microtubular THz resonance patterns at 1.0, 1.4, 2.2, and 2.4 THz. This is a structural inference, not a frequency-domain conversion from EEG data.}
    \label{fig:thz_frequencies}
\end{figure}
...
\end{longtable}
\begin{equation}
% TODO: Replace with the correct equation
\label{eq:37}
\end{equation}
